...
import { MapPin, Star, ChevronRight, Navigation, Coins, Banknote } from 'lucide-react';
...
export const PlaceCard: React.FC<PlaceCardProps> = ({
  ...
  const { navigate, calculateDistanceTo } = useApp();
            ...
            <div className="min-w-0 pr-2">
              ...
              <div className="flex items-center gap-2.5 mt-2.5 text-xs text-gray-300">
                ...
                <button
                  ...
                  title="Show the way and directions from current location"
                  className="flex items-center gap-1 text-emerald-300 hover:text-emerald-200 font-semibold cursor-pointer underline-offset-2 hover:underline"
                >
      ...
      >
        ...
        <div className="p-3.5 flex flex-col flex-1 justify-between">
          <div>
            ...
            <div className="flex items-center justify-between gap-1 mt-1 text-xs text-gray-500">
              <span>{place.categoryLabel}</span>
              {place.location && (
                <span className="text-emerald-700 font-medium text-[11px] truncate max-w-[110px]">
                  {place.location.split(',')[0]}
                </span>
              )}
            </div>

            {/* Whole Expense Breakdown Badge */}
            <div className="mt-2 p-2 rounded-xl bg-emerald-50/70 border border-emerald-100/90 text-xs">
              <div className="flex items-center justify-between font-bold text-gray-900 mb-0.5">
                <span className="text-[11px] text-[#005B49] flex items-center gap-1 font-extrabold">
              ...
              <div className="flex items-center justify-between text-[10px] text-gray-500 pt-0.5 border-t border-emerald-100/60">
                <span>Visit: <strong className="text-gray-700">{expenseSummary.visit.formatted}</strong></span>
                <span>•</span>
                <span>Travel: <strong className="text-gray-700">~{expenseSummary.travel.formattedRange}</strong></span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between mt-2.5 pt-2 border-t border-gray-50 text-xs">
            ...
            <button
              ...
              title="Show way and navigation from your location"
              className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-50 hover:bg-emerald-100/80 text-emerald-800 font-semibold text-[11px] transition-colors cursor-pointer border border-emerald-200/60"
            >
              <Navigation size={11} className="text-[#005B49] shrink-0" />
              <span className="truncate max-w-[130px]">{place.distance}</span>
            </button>
          </div>
        </div>
      </div>
